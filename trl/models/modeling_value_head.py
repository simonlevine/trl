...
class AutoModelForMaskedLMWithValueHead(PreTrainedModelWrapper):
    r"""
    A masked language model with a value head in addition to the language model head.
    This class inherits from `~trl.PreTrainedModelWrapper` and wraps a
    `transformers.PreTrainedModel` class. The wrapper class supports classic functions
    such as `from_pretrained`, `push_to_hub` and `generate`. To call a method of the wrapped
    model, simply manipulate the `pretrained_model` attribute of this class.
    ...
    Class attributes:
        - **transformers_parent_class** (`transformers.PreTrainedModel`) -- The parent class of the wrapped model. This
            should be set to `transformers.AutoModelForCausalLM` for this class.
        - **lm_head_namings** (`tuple`) -- A tuple of strings that are used to identify the language model head of the
            wrapped model. This is set to `("lm_head", "embed_out", "cls")` for this class but can be changed for other models
            in the future
        - **supported_args** (`tuple`) -- A tuple of strings that are used to identify the arguments that are supported
            by the `ValueHead` class. Currently, the supported args are:
            - **summary_dropout_prob** (`float`, `optional`, defaults to `None`) -- The dropout probability for the
    ...
    """
    transformers_parent_class = AutoModelForMaskedLM
    lm_head_namings = ["lm_head", "embed_out", "cls"]
    supported_args = (
        "summary_dropout_prob",
        "v_head_initializer_range",
        "v_head_init_strategy",
    ...
    def forward(
        ...
        **kwargs,
        ...
        if 'past_key_values' not in inspect.signature(self.pretrained_model.forward).parameters:
            kwargs.pop("past_key_values")
    ...
    def generate(self, *args, **kwargs):
        r"""
        We just call `forward` on the wrapped model and greedily sample logits.
        """
        return self.pretrained_model(*args, **kwargs).logits.argmax(-1)